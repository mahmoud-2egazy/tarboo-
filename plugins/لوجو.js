...
if (!effect) throw '*[❗] لستخدام الامر*\n—◉ _#لوجو [رقم اللوجو من(1)الي(199)][النص]_\n*مثال:*\n—◉ _#لوجو4 KLEVER \n\n*° ඬ⃟💌 #logo ' + effects.map(v => v.title).join('\n° ඬ⃟💌 #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗𝐈𝐍𝐅𝐎❗] التأثير ${effect} ليس في قائمة التأثيرات*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] التأثير ${effect} ليس في قائمة التأثيرات*` : `*[❗𝐈𝐍𝐅𝐎❗] الاستخدام الصحيح للأمر ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
